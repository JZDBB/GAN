...
import evaluate.face_distance_calculate as cal
devide = 100000000
...
class MyFrame(wx.Frame):
    ...
    def __init__(self, parent):
        ...
        self.m_staticText3 = wx.StaticText(self, wx.ID_ANY, u" ", wx.Point(120, 0), wx.Size(100, -1), 0)
        ...
        self.Bind(wx.EVT_BUTTON, self.OnClickCompute, self.m_compute)
    ...
    def OnClickSellect(self, evt):
        ...
        dlg.Destroy()
        filename = self.m_textCtrl1.GetValue()
        ...
        filename = filename.replace('inputs', 'targets')
    ...
    def OnClickGenarate(self, evt):
        ...
        self.outpath = filename.replace('inputs', 'outputs')
        plt.imsave(self.outpath, results['outputs'])
        image3 = wx.Image(self.outpath, wx.BITMAP_TYPE_ANY).Rescale(256, 256).ConvertToBitmap()
        self.m_imgout.SetBitmap(wx.BitmapFromImage(image3))

    def OnClickCompute(self, evt):
        targetpath = self.outpath.replace('outputs', 'targets')
        _, cossim = cal.calculate(targetpath, self.outpath)
        int_num = cossim * devide
        cossim = int(int_num)/ devide
        self.m_staticText3.SetLabel(str(cossim))
